...
pub fn generate_novelty_report(
    ...
    // Fetch all scrobbles in range - these come ordered by timestamp ASC
    ...
    for scrobble in &scrobbles {
        *artist_play_counts.entry(scrobble.artist.clone()).or_insert(0) += 1;
    }
    
    for discovery in &mut artist_discoveries {
        discovery.total_plays = artist_play_counts.get(&discovery.artist).copied().unwrap_or(0);
    }
...
mod tests {
    ...
    fn test_cumulative_tracking() {
        let scrobbles = vec![
            ...
            test_scrobble("2024-01-08T11:00:00Z", "Artist B", "Track 3"), // New
        ];
